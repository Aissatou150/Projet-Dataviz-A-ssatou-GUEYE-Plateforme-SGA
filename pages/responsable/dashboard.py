...
from models import Student, Course, Session as CS, Grade, Attendance

dash.register_page(__name__, path="/responsable/dashboard", title="Tableau de bord — Responsable")
...
def layout():
    return html.Div([html.Div(id="resp-dash-content")])


@callback(
    Output("resp-dash-content", "children"),
    Input("url", "pathname"),
    State("session-store", "data"),
)
def render(pathname, session):
    if pathname != "/responsable/dashboard":
    ...
    try:
        ...
        for c in courses_raw:
            ...
            nb_a  = sum(len(s.attendances) for s in sess)
            courses_data.append({
                "libelle": c.libelle,
                "done": done, "vol": c.volume_h, "pct": pct,
                "absences": nb_a,
            })
    ...
    prenom = (session or {}).get("prenom", "Responsable")
    ...
    # --- Bar absences par cours ---
    cours_labels = [c["libelle"] for c in courses_data]
    abs_vals     = [c["absences"] for c in courses_data]
    fig_abs = go.Figure(go.Bar(
        x=cours_labels, y=abs_vals,
        marker_color="rgba(239,68,68,0.72)",
        marker_line=dict(color="white", width=0.5),
        text=abs_vals, textposition="outside",
        textfont=dict(size=11),
        hovertemplate="%{x}: %{y} absence(s)<extra></extra>",
    ))
    ...
    return html.Div([
        ...
        html.Div([
            dbc.Row([
                ...
                dbc.Col(kpi(nb_abs,"Absences totales","#EF4444"), md=3, className="mb-3"),
            ...
            dbc.Row([
                ...
                dbc.Col([
                    html.Div([
                        html.Div("Absences par cours", className="card-header"),
                        html.Div(dcc.Graph(figure=fig_abs, config={"displayModeBar": False}),
                                 className="card-body"),
            ...
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.Div("Avancement des cours", className="card-header"),
                        html.Div(
